...
///
////////////////////////////////////////////////////////////////////////////////////////////
contract IllToken is ERC20, EIP712 {
  /// @dev version
  ...
  bytes32 internal constant _BACKUP_TRANSFER_SIGNATURE_TYPE_HASH =
    keccak256(
      'BackupTransfer(bytes32 warning,address from,uint amount,uint256 nonce)'
    );

  /// @dev event being triggered when backup transfer happens
  event TransferredToBackup(
    address _from,
    address _to,
    uint256 _amount,
  ...
  /// @dev setup a IllToken
  constructor(
    string memory _tokenName,
    string memory _tokenSymbol
  ) ERC20(_tokenName, _tokenSymbol) EIP712(_tokenName, version) {
  ...
  function transferToBackup(uint256 _amount) external {
    ...
    require(
      backupAddress[msg.sender] != address(0),
      'Backup address is not registered'
    );
